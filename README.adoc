...
This Python Kopf operator source-to-image can be used to build python-base operators using OpenShift source-to-image or Containerfile based builds.
...
This image may also be built with the provided `Containerfile`.

Using `podman`:

--------------------------------------------------------------
sudo podman build --tag python-kopf-s2i:latest -f ./Containerfile
--------------------------------------------------------------
...
This example image may also be built with the provided `Containerfile`.
The provided configuration builds from the base image `quay.io/rhpds/python-kopf-s2i:latest`.
In order to test your build of the s2i image you will need a modified version of the `Containerfile`:

--------------------------------------------------------------------------------
sed 's/^FROM .*/FROM python-kopf-s2i:latest/' example/Containerfile >example/Containerfile.custom
--------------------------------------------------------------------------------

Build using `podman`:

--------------------------------------------------------------------------------
sudo podman build --tag python-kopf-s2i-example:latest -f ./example/Containerfile.custom
--------------------------------------------------------------------------------

Using `docker`:

--------------------------------------------------------------------------------
sudo docker build -t python-kopf-s2i-example:latest -f ./example/Containerfile.custom example/
--------------------------------------------------------------------------------
